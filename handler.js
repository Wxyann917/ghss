...
/*      let levelAwal = Func.level(users.point)[0]
      ...
      if (((m.isGroup && !groupSet.mute) || !m.isGroup) && !users.banned) {
         if (body && body == isPrefix) {
            ...
            if (!banchat) {
               ...
               await client.reply(m.chat, Func.texted('bold', `Mengecek . . .`), m)
               return client.reply(m.chat, Func.texted('bold', `Kecepatan respon: ${((new Date - old) * 1)}ms (nonaktif)`), m)
            }
         }
      }
      ...
      if (isPrefix && !commands.includes(command) && matcher.length > 0 && !setting.self) {
         if (!m.isGroup || (m.isGroup && !groupSet.mute)) return client.reply(m.chat, `Perintah yang anda gunakan salah, coba rekomendasi berikut :\n\n${matcher.map(v => '➠ *' + (isPrefix ? isPrefix : '') + v.string + '* (' + v.accuracy + '%)').join('\n')}`, m)
      }
      if (body && isPrefix && commands.includes(command) || body && !isPrefix && commands.includes(command) && setting.noprefix || body && !isPrefix && commands.includes(command) && global.evaluate_chars.includes(command)) {
         ...
         if (setting.error.includes(command) && !setting.self) return client.reply(m.chat, Func.texted('bold', `Perintah ${(isPrefix ? isPrefix : '') + command} dinonaktifkan`), m)
         ...
         for (let name in is_commands) {
            ...
            if (setting.pluginDisable.includes(name)) return client.reply(m.chat, Func.texted('bold', `Fitur ini dinonaktifkan`), m)
            if (!m.isGroup && !['owner'].includes(name) && chats && !isPrem && !users.banned && new Date() * 1 - chats.lastchat < global.timer) continue
            if (!m.isGroup && !['owner'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `Saat ini owner sedang mengaktifkan mode grup, jadi bot hanya bisa digunakan di dalam group, silahkan join ke grup di bawah ini. jika ingin sewa bot untuk dimasukkan ke grup kamu, chat owner, ketik *#owner*.`, m, {
               largeThumb: true,
            ...
            if (cmd.restrict && !isOwner && text && new RegExp('\\b' + global.db.setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, `Anda melanggar *Syarat & Ketentuan* penggunaan bot dengan menggunakan kata kunci yang masuk daftar hitam, sebagai hukuman atas pelanggaran Anda diblokir dan dilarang. Untuk membuka blokir dan unbanned Anda harus membayar *Rp. 10.000,-*`, m).then(() => {
                  users.banned = true
            ...
            if (cmd.limit && users.limit < 1) {
               return client.reply(m.chat, `Penggunaan bot Anda telah mencapai limit dan akan disetel ulang setelah 12 jam.\n\nUntuk mendapatkan lebih banyak limit, tingkatkan ke paket premium kirim *#premium*`, m).then(() => users.premium = false)
               continue
            }
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && !['chatAI'].includes(name) && setting.groupmode) return client.sendMessageModify(m.chat, `Saat ini owner sedang mengaktifkan mode grup, jadi bot hanya bisa digunakan di dalam group, silahkan join ke grup di bawah ini. jika ingin sewa bot untuk dimasukkan ke grup kamu, chat owner, ketik *#owner*.`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg'),
               url: setting.link
            }).then(() => chats.lastchat = new Date() * 1)
