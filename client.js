...
const connect = async () => {
   ...
   global.client = Socket({
      ...
      browser: ['@_wtmsdrjt / hutao-bot', 'safari', '1.0.0'],
      auth: state,
      ...
      if (connection === 'connecting') {
       spinnies.add('start', {
         ...
         spinnies.succeed('start', {
            text: `Terkoneksi, kamu login sebagai ${client.user.name || client.user.verifiedName}`
         })
      } else if (connection === 'close') {
         if (lastDisconnect.error.output.statusCode == DisconnectReason.loggedOut) {
            spinnies.fail('start', {
               text: `Tidak bisa mengoneksi ke web socket`
            })
            await props.save()
            process.exit(0)
         } else {
