   ...
   async: async (m, {
      ...
      try {
         ...
         if (args && args[0] == '-y') {
            ...
            if (arr.length == 0) return client.reply(m.chat, `Alhamdulillah gaada sider.`, m)
            for (let jid of arr) {
               await Func.delay(2000)
               await client.groupParticipantsUpdate(m.chat, [jid], 'remove')
            }
            await client.reply(m.chat, `Selesai, ${arr.length} sider udah dihapus.`, m)
         } else {
            if (sider.length == 0 && lastseen.length == 0) return client.reply(m.chat, `Alhamdulillah gaada sider.`, m)
            let teks = ` *S I D E R*\n\n`
            teks += sider.length == 0 ? '' : `“List dari *${sider.length}* member yang tidak ada aktivitas.”\n\n`
            teks += sider.length == 0 ? '' : sider.map(v => '	◦  @' + v.replace(/@.+/, '')).join('\n')
            teks += '\n\n'
            teks += lastseen.length == 0 ? '' : `“List dari *${lastseen.length}* member ga onlen seminggu.”\n\n`
            teks += lastseen.length == 0 ? '' : lastseen.map(([v, x]) => '	◦  @' + v.replace(/@.+/, '') + '\n	     *Lastseen* : ' + Func.toDate(now - x.lastseen).split('D')[0] + ' days ago').join('\n')
            teks += `\n\n*Note* : Fitur ini akurat jika bot telah berada selama 1 minggu di grup, kirim *${isPrefix + command} -y* untuk mengeluarkan mereka.`
