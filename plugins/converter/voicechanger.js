...
exports.run = {
  ...
  // use: 'reply audio',
   ...
   async: async (m, {
      ...
      try {
         if (!m.quoted) return client.reply(m.chat, `Balas audio untuk menggunakan perintah ini.`, m)
         ...
         if (/audio/.test(mime)) {
            ...
            exec(`ffmpeg -i ${parse.file} ${set} ${ran}`, async (err, stderr, stdout) => {
               ...
               if (err) return client.reply(m.chat, `Waduh, gagal mengkonversikan.`, m)
            ...
            client.reply(m.chat, `Reply audio to use this command.`, m)
         }
      } catch (e) {
         console.log(e)
