      ...
      try {
         ...
         if (text) {
            ...
            if (m.quoted ? m.quoted.message : m.msg.viewOnce) {
               ...
               if (/video/.test(type)) {
                  if (q.seconds > 10) return client.reply(m.chat, `Maksimal video harus 10 detik wir`, m)
               ...
               if (/image\/(jpe?g|png)/.test(mime)) {
                  ...
                  if ((q.msg || q).seconds > 10) return client.reply(m.chat, `Maksimal video harus 10 detik wir`, m)
                  let img = await q.download()
                  if (!img) return client.reply(m.chat, global.status.wrong, m)
                  return await client.sendSticker(m.chat, img, m, {
                     packname: exif.sk_pack,
               ...
               } else client.reply(m.chat, `Kirim gambar/video pake caption ${isPrefix + command}`, m)
            }
         }
      } catch (e) {
         console.log(e)
