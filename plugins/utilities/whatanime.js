   ...
   async: async (m, {
      ...
      try {
         ...
         if (m.quoted ? m.quoted.message : m.msg.viewOnce) {
            ...
            if (!/image/.test(type)) return client.reply(m.chat, Func2.texted('bold', `Reply foto dengan perintah .whatanime`), m)
            let old = new Date()
            client.reply(m.chat, global.status.wait, m)
            let image = await Func2.uploadImage(img)
            ...
            if (!/image\/(jpe?g|png)/.test(mime)) return client.reply(m.chat, Func2.texted('bold', `Reply foto dengan perintah .whatanime`), m)
            let img = await q.download()
            if (!img) return client.reply(m.chat, Func2.texted('bold', `Berikan caption atau reply foto dengan perintah #whatanime`), m)
            let old = new Date()
            client.reply(m.chat, global.status.wait, m)
            let image = await Func2.uploadImage(img)
            let json = await scrap.whatanime(image)
